...
function Header() {
  ...
  useEffect(()=>{
    ...
    axios.get("http://localhost:8080/api/product").then(response=>setpro(response.data)).catch(err=>console.log(err));
  },)
...
axios.post("http://localhost:8080/api/sales",json).then(function(response){setwar(response.data)}).catch(function(error){console.log(error)});
...
setTimeout(()=>{
  ...
  axios.post("http://localhost:8080/api/shipment",ones).then(function(response){console.log(response)}).catch(function(error){console.log(error)});
},10000)
